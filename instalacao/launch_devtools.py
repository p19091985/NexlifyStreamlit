...
try:
    ...
    if not SCRIPT_CONFIG_INI.is_file():
        raise FileNotFoundError('config_banco_gui.py')
    if not SCRIPT_CREDENCIAIS.is_file():
        raise FileNotFoundError('gerador_credenciais_gui.py')
    if not SCRIPT_CONFIG_PY.is_file():
        raise FileNotFoundError('config_gui.py')
...
class LauncherApp(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        btn_config_ini = ttk.Button(main_frame, text='🗃️ Configurar Conexão (banco.ini)', command=lambda: launch_script(SCRIPT_CONFIG_INI))
        btn_config_ini.pack(fill='x', pady=5)
        btn_credenciais = ttk.Button(main_frame, text='🔑 Gerar Credenciais / Hashes', command=lambda: launch_script(SCRIPT_CREDENCIAIS))
        btn_credenciais.pack(fill='x', pady=5)
        btn_config_py = ttk.Button(main_frame, text='⚙️ Configurar Flags (config.py)', command=lambda: launch_script(SCRIPT_CONFIG_PY))
