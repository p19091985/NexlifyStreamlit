...
try:
    ...
    messagebox.showerror('Erro Crítico', f"Erro ao localizar/criar 'config_settings.ini' na pasta raiz do projeto: {e}")
    sys.exit(1)
...
class ConfigApp(tk.Tk):
    ...
    def __init__(self, config_path):
        ...
        self.vars_map = {'database_enabled': self.db_enabled_var, 'initialize_database_on_startup': self.init_db_var, 'use_login': self.use_login_var, 'redirect_console_to_log': self.redirect_log_var}
    ...
    def _create_widgets(self):
        ...
        ttk.Label(main_frame, text='Configurações Principais (config_settings.ini)', style='Header.TLabel').pack(pady=(0, 15), anchor='w')
        db_frame = ttk.LabelFrame(main_frame, text='🎲 Conexão Principal', padding=15)
        db_frame.pack(fill=tk.X, expand=False, pady=5)
        self.db_enabled_check = ttk.Checkbutton(db_frame, text='Habilitar Banco de Dados (database_enabled)', variable=self.db_enabled_var, command=self._on_db_setting_change)
        self.db_enabled_check.pack(anchor='w')
        ttk.Label(db_frame, text='Controla se a aplicação se conecta a um banco de dados.', style='Help.TLabel', wraplength=500).pack(anchor='w', padx=20, pady=(0, 5))
        ...
        self.init_db_check = ttk.Checkbutton(dependent_frame, text='Inicializar Banco ao Iniciar (initialize_database_on_startup)', variable=self.init_db_var)
        self.init_db_check.pack(anchor='w', pady=(5, 0))
        ttk.Label(dependent_frame, text='Se ativo, cria o schema do banco (SQLite). Depende do DB Habilitado.', style='Help.TLabel', wraplength=500).pack(anchor='w', padx=20, pady=(0, 5))
        self.use_login_check = ttk.Checkbutton(dependent_frame, text='Exigir Login de Usuário (use_login)', variable=self.use_login_var)
        self.use_login_check.pack(anchor='w', pady=(5, 0))
        ttk.Label(dependent_frame, text='Se ativo, exige autenticação. Depende do DB Habilitado.', style='Help.TLabel', wraplength=500).pack(anchor='w', padx=20, pady=(0, 5))
        ...
        self.redirect_log_check = ttk.Checkbutton(general_frame, text='Redirecionar Console para Log (redirect_console_to_log)', variable=self.redirect_log_var)
        self.redirect_log_check.pack(anchor='w')
        ttk.Label(general_frame, text='Se ativo, a saída do console (print) vai para arquivos de log.', style='Help.TLabel', wraplength=500).pack(anchor='w', padx=20, pady=(0, 5))
        self.save_button = ttk.Button(main_frame, text=self.original_save_button_text, command=self._save_settings, style='Success.TButton')
    ...
    def _on_db_setting_change(self, *args):
        ...
        if is_db_enabled:
            ...
            self.dependency_status_label.config(text='Banco HABILITADO. Opções dependentes disponíveis.', style='Success.TLabel')
        else:
            ...
            self.dependency_status_label.config(text='Banco DESABILITADO. Login e Inicialização foram desativados.', style='Warning.TLabel')
    ...
    def _load_initial_values(self):
        try:
            ...
            messagebox.showerror('Erro de Leitura INI', f"Erro ao processar '{self.config_path}':\n{e}", parent=self)
            ...
            self.save_button.config(state='disabled')
            self._update_status(f'Erro geral ao ler .ini: {e}', 'red')
        self._on_db_setting_change()
