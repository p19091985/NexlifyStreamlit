...
BLOCK_DEFINITIONS = {'sqlserver': ['type', 'host', 'port', 'dbname', 'user', 'password'], 'sqlite': ['type', 'path'], 'postgresql': ['type', 'host', 'port', 'dbname', 'user', 'password'], 'mysql': ['type', 'host', 'port', 'dbname', 'user', 'password'], 'mariadb': ['type', 'host', 'port', 'dbname', 'user', 'password']}
...
class DatabaseConfigManager:
    ...
    def load_config(self):
        ...
        self.blocks = []
        self.current_active_block_name = None
        type_regex = re.compile('^\\s*(#)?\\s*type\\s*=\\s*(\\w+)')
        key_val_regex = re.compile('^\\s*(#)?\\s*(\\w+)\\s*=')
        block_ranges = []
        ...
        for i, line in enumerate(self.lines):
            ...
            if type_match:
                if current_block_start != -1:
                    block_ranges.append({'type': current_db_type, 'start': current_block_start, 'end': i, 'is_commented': current_is_commented, 'line_num': current_block_start + 1})
                current_is_commented = type_match.group(1) is not None
                current_db_type = type_match.group(2)
                current_block_start = i
        if current_block_start != -1:
            block_ranges.append({'type': current_db_type, 'start': current_block_start, 'end': len(self.lines), 'is_commented': current_is_commented, 'line_num': current_block_start + 1})
...
class App(tk.Tk):
    ...
    def _update_radio_buttons(self):
        ...
        for db_name in all_dbs:
            rb = ttk.Radiobutton(self.radio_frame, text=db_name, variable=self.selected_db, value=db_name)
