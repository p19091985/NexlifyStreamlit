...
try:
    ...
    messagebox.showerror('Erro Crítico de Importação', f"Não foi possível importar os módulos de segurança: {e}\n\nCertifique-se de que este script está na pasta 'instalacao' e que a pasta 'persistencia' existe na raiz do projeto.")
    sys.exit(1)
...
class CredentialToolApp(tk.Tk):
    ...
    def _create_ini_tab(self, parent):
        ...
        ini_show_pass_check = ttk.Checkbutton(input_frame, text='Mostrar senha', variable=self.ini_show_pass_var, command=self._toggle_ini_pass)
        ini_show_pass_check.grid(row=2, column=1, padx=5, pady=(0, 10), sticky='w')
        ttk.Button(input_frame, text='Gerar Credenciais', command=self._generate_ini_creds, style='Success.TButton').grid(row=3, column=0, columnspan=2, pady=10, sticky='ew', ipady=5)
        ...
        ttk.Button(output_frame, text='Copiar Resultado', command=self._copy_ini_output).grid(row=1, column=0, pady=(10, 0), sticky='ew', ipady=5)
    ...
    def _create_sql_tab(self, parent):
        ...
        sql_show_pass_check = ttk.Checkbutton(input_frame, text='Mostrar senha', variable=self.sql_show_pass_var, command=self._toggle_sql_pass)
        sql_show_pass_check.grid(row=1, column=1, padx=5, pady=(0, 10), sticky='w')
        ttk.Button(input_frame, text='Gerar Hash', command=self._generate_sql_hash, style='Success.TButton').grid(row=2, column=0, columnspan=2, pady=10, sticky='ew', ipady=5)
        ...
        ttk.Button(output_frame, text='Copiar Hash', command=self._copy_hash_output).grid(row=1, column=0, pady=(5, 0), sticky='ew', ipady=5)
