...
st.set_page_config(page_title='Painel Análise Iris', layout='wide', page_icon='📈')
st_check_session()
try:
...
def get_interactive_eda_figs(_df):
    ...
    figs = []
    fig1 = px.scatter(df, x='petal length (cm)', y='petal width (cm)', color='species', title='Comprimento vs. Largura da Pétala')
    figs.append(fig1)
    fig2 = px.scatter(df, x='sepal length (cm)', y='sepal width (cm)', color='species', title='Comprimento vs. Largura da Sépala')
    figs.append(fig2)
    count_df = df['species'].value_counts().reset_index()
    count_df.columns = ['species', 'count']
    fig3 = px.bar(count_df, x='species', y='count', color='species', title='Contagem por Espécie')
    figs.append(fig3)
    fig4 = px.box(df, x='species', y='sepal length (cm)', color='species', title='Box Plot do Comprimento da Sépala')
    figs.append(fig4)
    fig5 = px.violin(df, x='species', y='petal width (cm)', color='species', title='Violin Plot da Largura da Pétala')
    figs.append(fig5)
    fig6 = px.histogram(df, x='petal length (cm)', color='species', title='Distribuição do Comprimento da Pétala')
    figs.append(fig6)
    return figs
...
def get_interactive_pca_fig(_df):
    try:
        ...
        fig_pca = px.scatter_3d(pca_df, x='1º Autovetor', y='2º Autovetor', z='3º Autovetor', color='species', title='Primeiras três dimensões do PCA (Interativo)')
        return fig_pca
    except Exception as e:
...
def render_uploader_panel():
    with st.expander('Passo 1: Configuração da Análise', expanded=True):
        ...
        uploaded_file = st.file_uploader('Selecione um arquivo CSV (Opcional)', type='csv', help=f"Se nenhum arquivo for enviado, o dataset padrão '{default_csv_path}' será usado.", key='iris_uploader_main')
...
def render_main_panel():
    ...
    with st.expander('Passo 2: Análise Gráfica Interativa', expanded=True):
        ...
        st.button('Iniciar Análise Gráfica (EDA + PCA)', on_click=run_analysis_callback, type='primary', disabled=st.session_state.full_df is None)
